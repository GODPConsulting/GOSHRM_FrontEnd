...
@Component({
  selector: 'app-add-emp-dependent',
  templateUrl: './add-emp-dependent.component.html',
  styleUrls: ['./add-emp-dependent.component.scss']
})
export class AddEmpDependentComponent implements OnInit {
  ...
  public initIncidentForm(): void {
    this.incidentForm = this.fb.group({
      pageContentId: [
        this.data?.isEditing ? this.data?.editObject?.pageContentId : 0,
        Validators.required,
      ],
      page_Name: [
        this.data?.isEditing ? this.data?.editObject?.page_Name : '',
        Validators.required,
      ],
      page_Title: [
        this.data?.isEditing ? this.data?.editObject?.page_Title : '',
        Validators.required,
      ],
      section_Content: [
        this.data?.isEditing ? this.data?.editObject?.section_Content : '',
        Validators.required,
      ],
  ...
  public submit(): void {
    ...
    if (this.incidentForm.valid) {
      ...
      this._helper.startSpinner();
        ...
        this._content.addUpdateContent(payload).subscribe({
          next: (res: any) => {
